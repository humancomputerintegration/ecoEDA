...
This file has been modified to support ecoEDA purposes. It needs to be edited by the user in order to work.
"""

#changed file located in /Applications/KiCad/KiCad.app/Contents/SharedSupport/scripting/kicad_pyshell (Mac) or C:\Program Files\KiCad\7.0\share\kicad\scripting\kicad_pyshell (Windows)
...
class KiCadPyShell(KiCadEditorNotebookFrame):
    def __init__(self, parent):
        ...
        ## LOCATION TO CHANGE FOR ECOEDA, note for Windows: make sure to use \\ for \
        self.ecoEDA_dir = "/Users/jasminelu/Documents/00_Research Projects/ecoEDA/ecoEDA-interface/kicad/"
        self.fp_dir = "/Applications/KiCad/KiCad.app/Contents/SharedSupport/footprints/"
    ...
    def _setup_startup(self):
        if self.config_dir != "":
            ...
            self.startup_file = os.path.join(self.ecoEDA_dir,"PyShell_eeschema_startup.py")

            
            with open(self.startup_file) as f:
                lines = f.readlines()
                if os.name == 'nt':
                    lines[1] = "ecoEDA_dir = \"" + repr(self.ecoEDA_dir)[1:-1] + "\"\n"
                    lines[3] = "fp_dir = \"" + repr(self.fp_dir)[1:-1] + "\"\n"
                else:
                    lines[1] = "ecoEDA_dir = \"" + self.ecoEDA_dir + "\"\n"
                    lines[3] = "fp_dir = \"" + self.fp_dir + "\"\n"
    ...
    def _setup(self):
        ...
        ## CODE CHANGED FOR ECOEDA FOR ECOEDA
        if os.name == 'nt':
            self.shell.run("ecoEDA_dir=\"" + repr(self.ecoEDA_dir)[1:-1] + "\"")
        else:
            self.shell.run("ecoEDA_dir=\"" + self.ecoEDA_dir + "\"")
        self.shell.runfile(self.ecoEDA_dir + "ecoEDA_pyshell_init.py")
